...
Function OnPeriodicUpdate(float hoursPassed)
	...
	ModAttribute("_Datt_Pride",-1 * prideHit * prideHitReduce * hoursPassed)
EndFunction
...
Function OnPlugVibration(float vibrationStrength)
	...
	Libs.PlayerRef.DamageAV("Magicka", ((vibrationStrength * 0.2) + (playerArousal / 50.0)) * (1.0 - (nympho / 100.0)))
...
Function CheckAndEquipArmorEffects(Armor akArmor)
    ...
    ElseIf (akArmor.HasKeyword(MonitorQuest.Libs.Zad.zad_DeviousBoots))
    	If(akArmor.HasKeyword(MonitorQuest.Libs.deviousPonyBoots))
    		MonitorQuest.OnPonyBootsEquipped("deviousPonyBoots","",1.0,None)
    	Else
...
Function CheckAndUnequipArmorEffects(Armor akArmor)
 ;special item tags -> extra effects
    ...
    ElseIf (akArmor.HasKeyword(MonitorQuest.Libs.Zad.zad_DeviousBoots))
    	If(akArmor.HasKeyword(MonitorQuest.Libs.deviousPonyBoots))
    		MonitorQuest.OnPonyBootsUnequipped("deviousPonyBoots","",1.0,None)
    	Else
